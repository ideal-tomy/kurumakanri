...
import { buildPortalPreviewFromSendItem } from '@/lib/customer-portal-preview';
import {
...
export type ReviewPayloadChannel = 'LINE' | 'MAIL' | 'BOTH';

export type ReviewPayloadQuoteBlock = {
  id: string;
...
export type ReviewPayloadItem = {
  ...
  portal_preview: CustomerPortalData | null;
  warnings: string[];
};
...
export async function buildReviewPayloadItems(
  supabase: SupabaseClient,
  args: {
    customer_ids: string[];
    rule: ReviewPayloadRule;
    channel: ReviewPayloadChannel;
    template_key?: string;
  },
): Promise<{ template_key: string; channel: ReviewPayloadChannel; items: ReviewPayloadItem[] }> {
  const templateKey = resolveNotificationTemplateKey(args.rule, args.template_key);
  const ch = args.channel;
  const wantLine = ch === 'LINE' || ch === 'BOTH';
  const wantMail = ch === 'MAIL' || ch === 'BOTH';
  const isOilRule = args.rule === 'oil_4000km';

  const items: ReviewPayloadItem[] = [];

  for (const customerId of args.customer_ids) {
    ...
    if (!overview) {
      items.push({
        ...
        vehicle_id: null,
        plate: null,
        rule_label: args.rule === 'custom' ? 'カスタム' : ruleLabelFor(args.rule as RuleKey),
        line_preview: null,
        mail_subject: null,
        mail_body: null,
        quote_link_preview: null,
        portal_link_preview: null,
        quote: null,
        portal_preview: null,
    ...
    const varsForQuoteLink = await buildMessageVariables(overview, {
      channel: wantLine ? 'LINE' : 'MAIL',
    });
    const quoteLinkPreview = varsForQuoteLink.quoteUrl ?? null;
    const portalLinkPreview = varsForQuoteLink.portalUrl ?? null;

    let quoteBlock: ReviewPayloadQuoteBlock | null = null;
    ...
    const item: ReviewPayloadItem = {
      ...
      portal_link_preview: portalLinkPreview,
      quote: quoteBlock,
      portal_preview: null,
      warnings,
    };

    item.portal_preview = buildPortalPreviewFromSendItem(
      {
        customer_id: item.customer_id,
        vehicle_id: item.vehicle_id,
        plate: item.plate,
        name: item.name,
        line_preview: item.line_preview,
        mail_subject: item.mail_subject,
        mail_body: item.mail_body,
        warnings: item.warnings,
        quote_link_preview: item.quote_link_preview,
        portal_link_preview: item.portal_link_preview,
        quote: quoteBlock,
      },
      overview,
    );
  ...
  return { template_key: templateKey, channel: ch, items };
}
